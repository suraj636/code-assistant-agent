...
def dispatch_tools(response_content: list, dispatch: dict, ui_callbacks: Optional[dict] = None) -> List[Dict[str, Any]]:
    """
    ...
    for block in response_content:
        ...
        if ui_callbacks and "on_tool_call" in ui_callbacks:
            ui_callbacks["on_tool_call"](tool_name, tool_input)
        else:
        ...
        if ui_callbacks and "on_tool_result" in ui_callbacks:
            ui_callbacks["on_tool_result"](tool_name, output)
        else:
...
def stream_loop(
    ...
    ui_callbacks: Optional[dict] = None
):
    """
    ...
    while True:
        ...
        if ui_callbacks and "on_status" in ui_callbacks:
            ui_callbacks["on_status"]("> Thinking...")
        else:
        ...
        with client.messages.stream(
            ...
            **extra_kwargs,
            ...
            for text in stream.text_stream:
                if ui_callbacks and "on_token" in ui_callbacks:
                    ui_callbacks["on_token"](text)
                else:
        ...
        if not (ui_callbacks and "on_token" in ui_callbacks):
            print()
        ...
        results = dispatch_tools(response.content, dispatch, ui_callbacks)
